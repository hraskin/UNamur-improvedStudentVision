...
class Camera:
    def __init__(self):
        self.cap = None
        self.landmarks_to_draw = []  # stocke les landmarks pour dessin
        self.handedness_to_draw = [] # stocke la main gauche/droite

        # Configuration du modèle en mode LIVE_STREAM (asynchrone)
    ...
    def show_feed(self, title="Camera"):
        ...
        while True:
            ...
            # Envoyer la frame à chaque tick
            mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=frame)
            timestamp_ms = int(cv2.getTickCount() / cv2.getTickFrequency() * 1000)
            self.detector.detect_async(mp_image, timestamp_ms=timestamp_ms)

            # Dessiner les résultats du dernier callback
            if self.landmarks_to_draw:
